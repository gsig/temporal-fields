...
function M.parse(arg)
   ...
   cmd:option('-actionmap',  './Charades_v1_mapping.txt', 'Path to action->obj/verb map')
   ...
   cmd:option('-cache_size',  50,      'mini-batch size (1 = pure stochastic)')
   cmd:option('-dumpLocalize','true',  'Output localization')
   cmd:option('-message_passes',   10,      'number of message passes in the test phase')
   cmd:option('-testOnly',    'false', 'Run on validation set only')
   cmd:option('-fixGoal',     'false', 'Never update goal (true | false)')
   cmd:option('-tenCrop',     'false', 'Ten-crop testing')
   cmd:option('-accumGrad',   4,       'Accumulate gradient accross N batches (Increase effective batch size)')
   cmd:option('-videoSize',   1, 'Effective Size of video CRF (message scaling factor)')
   cmd:option('-videoSizeGoal',1, 'Effective Size of video CRF (message scaling factor)')
   ...
   cmd:option('-messageDecay', 0.9,   'message decay [0-1] (0.9 default)')
   cmd:option('-convLR',       1.0,   'convolution layers LR modifier')
   cmd:option('-fc8LR',        1.0,   'fc8 layer LR modifier')
   ---------- Model options ----------------------------------
   cmd:option('-netType',      'crf_atf', 'Options: vgg16 | crf3 | crf_atf')
   cmd:option('-pretrainpath', './',     'Path to pretrained models')
   cmd:option('-depth',        34,       'ResNet depth: 18 | 34 | 50 | 101 | ...', 'number')
   cmd:option('-goals',        10,       'Size of latent goal space')
   cmd:option('-marginal',     'mean',   'How to predict whole video')
   cmd:option('-fc7_dropout',  0.5,       'Dropout rate after fc7 [0,1]')
   cmd:option('-intent_decay', 1e-3,  'intent weight decay (prior)')
   cmd:option('-sigma',        150,      'Stdev of temporal distance kernel')
   cmd:option('-shortcutType', '',       'Options: A | B | C')
   cmd:option('-uniquest',     'false',  'Uniquest [true|false]')
   ...
   opt.fixGoal = opt.fixGoal ~= 'false'
   opt.uniquest = opt.uniquest ~= 'false'
   ...
   elseif opt.dataset == 'charades_more' then
      if not paths.dirp(opt.data) then
         cmd:error('error: missing Charades data directory')
      end
      opt.nEpochs = opt.nEpochs == 0 and 1 or opt.nEpochs
   elseif opt.dataset == 'charadesflow_more' then
      if not paths.dirp(opt.data) then
         cmd:error('error: missing Charades flow data directory')
      end
      opt.nEpochs = opt.nEpochs == 0 and 1 or opt.nEpochs
   else
